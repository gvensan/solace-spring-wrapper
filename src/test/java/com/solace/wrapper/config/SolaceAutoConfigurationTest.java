...
/**
 * Unit tests for {@link SolaceAutoConfiguration}: bean factory methods (publisher, consumer manager,
 * requestor) and the {@code SolaceMetricsGaugeBinder} that registers connection/active-endpoint
 * gauges. Uses a non-connecting connection manager and stub providers — no broker required.
 */
class SolaceAutoConfigurationTest {
    ...
    void bean_factory_methods_build_components() {
        ...
        SolaceRequestor requestor = cfg.solaceRequestor(cm, serializer, rr, provider(metrics));
